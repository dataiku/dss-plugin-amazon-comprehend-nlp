# -*- coding: utf-8 -*-
"""Module with utility functions to call the Amazon Comprehend API"""
...
def batch_api_response_parser(batch: List[Dict], response: Union[Dict, List], output_column_names: NamedTuple) -> Dict:
    """
    ...
    for i in range(len(batch)):
        for k in output_column_names:
            batch[i][k] = ""
        result = [r for r in results if str(r.get("Index", "")) == str(i)]
        error = [r for r in errors if str(r.get("Index", "")) == str(i)]
        if len(result) != 0:
            # result must be json serializable
            batch[i][output_column_names.response] = json.dumps(result[0])
        if len(error) != 0:
            inner_error = error[0]
            logging.warning(str(inner_error))
            # custom for Azure edge case which is highly nested
            batch[i][output_column_names.error_message] = inner_error.get("ErrorMessage", "")
            batch[i][output_column_names.error_type] = inner_error.get("ErrorCode", "")
            batch[i][output_column_names.error_raw] = str(inner_error)
    return batch
